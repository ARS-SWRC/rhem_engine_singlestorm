...
C** 4/03  0. = zro
      ...
      dimension t(2000), r(2000), z(2000), upq(10), slope(2),fav(20),
...
C                                                         connecting impervious
        ...
        if (upflow) call old (-888, 'RO', up888, nq, ierr)
...
C                                                iterative solution for h2(j+1)
                h2jp1 = (h2(j) + h1(jp1)) / 2.
                source = 'UrbPLh'
                call iter (errpl, h2jp1, 0., h_lim, ierr, source)
...
C                                                               upstream inflow
      do k = 1, nu
        call old (idup(k), 'RO', upq(k), nq, ierr)
        if (ierr .gt. 0) call errxit
...
C------------------------------------------------------------------------------
            ...
            source = 'UrbUBs'
            call iter (errsub, h2(1), 0., h_lim, ierr, source)
...
C------------------------------------------------------------------------------
            ...
            j = 2
            source = 'UrbZAS'
            call iter (errsza, h2a, 0., h_lim, ierr, source)
...
C------------------------------------------------------------------------------
          ...
          do j = 1, nk - 1
            ...
            h2jp1 = h2(jp1)
            source = 'UrbH2S'
            call iter (errst, h2jp1, 0., h_lim, ierr, source)
...
C**  this is a dummy call to put in zeros
        call swrt (wso, 0., 0., 0., 0., 0., outc, 0., 0.)
      end if
