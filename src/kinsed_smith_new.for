      ...
      do j = 1, nchan
        do i = 2, nk
          if (depnet(i,j) .lt. 0.)then
              wse_neg = wse_neg + depnet(i,j)
          else 
              wse_pos = wse_pos + depnet(i,j)
          end if
        end do
      end do
...
C        if (ltyp .eq. plane) qfac = qf
      ...
      if (nchan .eq. 2 .and. nlat .gt. nm .and. ltyp .ne. pipe) then
...
C------------------------------------------------------------------------------
        ...
        do n = 1, nps
          ...
          call old (j, attr(n,1), latc(n,nl), nc, ierr)
