...
C  the following is new criterea [10/01]
              ...
              if(fs(j,i) .le. 1.e-5) then
                famx = vmin*2.*(1.+sqrt(0.5*apu/dtu/vmin))  ! new limit
              end if
...
C
              ...
              tsn = tsl + dtu*vmu/apu
              vsn = tsn + sqrt(2.*tsn)    ! normalized philip 
...
C!!
                  ...
                  tsn = tsl + dtu*vmu/apu
                  vns = tsn + sqrt(2.*tsn)    ! normalized philip
...
C                                                              for crusted layered case
            rc(i) = sk1(i) * (1. + alf / (exp (alf * def1(i)*
...
C!! new checks for overflo:  12/02
           ...
           if(xtermL .gt. 80. .or. xtermN .gt. 80.) then
             ...
             FG2 = exp(xtermN)          ! substitute xterms
             ratl = (FG2 - 1. + alf)/(exp(xtermL) - 1. + alf)
  ...
  99  format(' neg. F2L or logf invalid in errvf: ',2i4,4g14.4)
             ...
             ferr = F2n - F2l - apu*log(ratl) - vmin*dtu*(1.- alf)
...
C!!-------------------------------------------------------------new 12/02
      function favdI( Infim, Apu, AK, Iv, gam, dt)
C finds increment of potential infil depth in interval dt
      real :: Iv, Is1, Is2, Isz  !, gam = 0.8
      integer :: Infim
...
C---------------------------------------------
      function fstar (Infc, Ic, apu, alf )
        integer :: Infc
        real    :: apu, alf, fe, Ic, Istar
C
        select case (Infc)
C
          case(1)
...
C----------------------------------------------
      function dfstar ( Infc, Ic, Apu, alf )
        integer :: Infc
        real    :: apu, alf, fe, Ic
C
        select case (Infc)
...
C----------------------------------------------
C
      function fcore (Infc, Ic, Apu, alf )
        integer :: Infc
        real    :: apu, alf, fe, Ic
C
        select case (Infc)
